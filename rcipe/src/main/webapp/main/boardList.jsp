...
<script>
	...
	function fncGetList(currentPage) {
		...
		alert('currentPage : ' + document.getElementById("currentPage").value
				...
				.ajax(
						"getBoardList",
						{
							...
							success : function(result) {
								...
								<c:if test="${ resultPage.currentPage > resultPage.pageUnit }">
								  $("#pagingfooter").append("<a href='javascript:fncGetList('"+(resultPage.currentPage-1)+"')'>◀ 이전</a>");
							  </c:if>
							  
							  <c:forEach var="i"  begin="1" end="3" step="1">    // 값이 안받아짐
							  // <c:forEach var="i"  begin="${resultPage.beginUnitPage}" end="${resultPage.endUnitPage}" step="1">    // 값이 안받아짐
							  
							  $("#pagingfooter").append("<a href='javascript:fncGetList(${ i });'>${ i }</a>");
							  </c:forEach>
							 ...
							 <c:if test="${ resultPage.endUnitPage < resultPage.maxPage }">
							 $("#pagingfooter").append("<a href='javascript:fncGetList('"+(resultPage.endUnitPage+1)+"')'>이후 ▶</a>");
							 </c:if>
